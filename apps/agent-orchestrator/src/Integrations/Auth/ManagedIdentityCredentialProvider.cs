...
namespace Atlas.AgentOrchestrator.Integrations.Auth;

/// <summary>
/// Provides managed identity credentials for Azure SDK clients in agent orchestrator.
///
/// Two credential paths are available to callers; choose the appropriate one explicitly:
///   <see cref="GetCredential"/>              — user-assigned MI (ACR, Key Vault, AI Foundry, Carbon Optimization)
///   <see cref="GetSystemAssignedCredential"/> — system-assigned MI (Resource Graph, Monitor, Cost Management)
///
